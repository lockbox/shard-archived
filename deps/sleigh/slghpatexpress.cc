...
int4 TokenPattern::resolveTokens(const TokenPattern &tok1,
                                 const TokenPattern &tok2)
...
{ // Use the token lists to decide how the two patterns
  ...
  rightellipsis = false;
  int4 ressa = 0;
  int4 minsize = tok1.toklist.size() < tok2.toklist.size()
                     ? tok1.toklist.size()
                     : tok2.toklist.size();
  if (minsize == 0) {
    // Check if pattern doesn't care about tokens
    if ((tok1.toklist.size() == 0) && (tok1.leftellipsis == false) &&
        (tok1.rightellipsis == false)) {
      toklist = tok2.toklist;
      leftellipsis = tok2.leftellipsis;
      rightellipsis = tok2.rightellipsis;
      return 0;
    } else if ((tok2.toklist.size() == 0) && (tok2.leftellipsis == false) &&
  ...
  if (reversedirection) {
    for (int4 i = 0; i < minsize; ++i)
      if (tok1.toklist[tok1.toklist.size() - 1 - i] !=
    ...
    for (int4 i = 0; i < minsize; ++i)
      if (tok1.toklist[i] != tok2.toklist[i]) {
        ostringstream msg;
        msg << "Mismatched tokens when combining patterns -- " << dec
            << tok1.toklist[i] << " != " << dec << tok2.toklist[i];
        throw SleighError(msg.str());
      }
  }
...
PatternBlock *TokenPattern::buildSingle(int4 startbit, int4 endbit,
                                        uintm byteval)
...
PatternBlock *TokenPattern::buildBigBlock(int4 size, int4 bitstart, int4 bitend,
                                          intb value)
...
PatternBlock *TokenPattern::buildLittleBlock(int4 size, int4 bitstart,
                                             int4 bitend, intb value)
...
{ // Build pattern block given a littleendian contiguous
  ...
  // bit indices label the least sig bit as 0 into a bit range on big endian
  // bytes where the indices label the most sig bit as 0.  The reversal due to
  ...
  // leave the upper bits the same, but transform the lower 3-bit value x into
  // 7-x.

  startbit =
      (bitstart / 8) * 8; // Get the high-order portion of little/LSB labelling
  endbit = (bitend / 8) * 8;
...
{ // A basic context pattern
  ...
  block = buildBigBlock(size, size * 8 - 1 - endbit, size * 8 - 1 - startbit,
                        value);
...
PatternExpression *PatternExpression::restoreExpression(const Element *el,
                                                        Translate *trans)
...
static intb getInstructionBytes(ParserWalker &walker, int4 bytestart,
                                int4 byteend, bool bigendian)
...

{
  ...
  SubtableSymbol *tab =
      dynamic_cast<SubtableSymbol *>(sleigh->findSymbol(tabid));
...
intb PlusExpression::getSubValue(const vector<intb> &replace,
                                 int4 &listpos) const
...
intb SubExpression::getSubValue(const vector<intb> &replace,
                                int4 &listpos) const
...
intb MultExpression::getSubValue(const vector<intb> &replace,
                                 int4 &listpos) const
...
{
  intb leftval = getLeft()->getValue(walker);
  int1 rightval = (int1)getRight()->getValue(walker);
  __int128_t shifted_val = (__int128_t)(leftval << (rightval & 0x3f));

  return (intb)(shifted_val);
}

intb LeftShiftExpression::getSubValue(const vector<intb> &replace,
                                      int4 &listpos) const

{
  intb leftval = getLeft()->getSubValue(replace, listpos); // Must be left first
  intb rightval = getRight()->getSubValue(replace, listpos);
  __int128_t shifted_val = (__int128_t)leftval << rightval;
  return (intb)(((__int128_t)leftval) << rightval);
}
...
intb RightShiftExpression::getSubValue(const vector<intb> &replace,
                                       int4 &listpos) const
...
intb AndExpression::getSubValue(const vector<intb> &replace,
                                int4 &listpos) const
...
intb XorExpression::getSubValue(const vector<intb> &replace,
                                int4 &listpos) const
...
intb DivExpression::getSubValue(const vector<intb> &replace,
                                int4 &listpos) const
...
intb MinusExpression::getSubValue(const vector<intb> &replace,
                                  int4 &listpos) const
...
intb NotExpression::getSubValue(const vector<intb> &replace,
                                int4 &listpos) const
...
static bool advance_combo(vector<intb> &val, const vector<intb> &min,
                          vector<intb> &max)
...
static TokenPattern buildPattern(PatternValue *lhs, intb lhsval,
...
void OperandEquation::operandOrder(Constructor *ct,
                                   vector<OperandSymbol *> &order) const
...

{
  ...
  if (resultpattern.getLeftEllipsis() ||
      resultpattern.getRightEllipsis()) // don't know length
    state.size = -1;
  else
  ...
  do {
    ...
    for (lhsval = lhsmin; lhsval <= lhsmax; ++lhsval) {
      ...
      if (count == 0)
        ...
        resultpattern =
            resultpattern.doOr(buildPattern(lhs, lhsval, semval, cur));
      count += 1;
    }
  } while (advance_combo(cur, min, max));
  if (count == 0)
...
void EquationAnd::operandOrder(Constructor *ct,
                               vector<OperandSymbol *> &order) const
...
void EquationOr::operandOrder(Constructor *ct,
                              vector<OperandSymbol *> &order) const
...

{
  ...
  if ((!left->getTokenPattern().getLeftEllipsis()) &&
      (!left->getTokenPattern().getRightEllipsis())) {
    // Keep the same base
    state.offset +=
        left->getTokenPattern().getMinimumLength(); // But add to its size
...
void EquationCat::operandOrder(Constructor *ct,
                               vector<OperandSymbol *> &order) const
...
void EquationLeftEllipsis::operandOrder(Constructor *ct,
                                        vector<OperandSymbol *> &order) const
...
void EquationRightEllipsis::operandOrder(Constructor *ct,
                                         vector<OperandSymbol *> &order) const
